...
#---------#         #-#          #---------#
# Penalization Amount
lambda.AFreg.param.homotopic<-0.9 # Between 0 and 1; 0 no change 1 (AF)
...
if((!exists("long.term.level") | RECALIBRATEPARAMETERS.Q)){
  ...
  long.term.level<-Estimated.OU.Parameters.Betas$Mu
  # (Estimated) Mean-Reversion Speed
  mean.reversion.speed<-Estimated.OU.Parameters.Betas$Theta
  # (Estimated)  Volatility Matrix
  Vol.mat.estim<-Estimated.OU.Parameters.Betas$Sigma
...
NS.samples<-matrix(data=NA,ncol=4,nrow=N.mat)
colnames(NS.samples)<-c("T.Mat","b1","b2","b3")

# Populate Matrix
# Initialize Progress Bar
pb <- txtProgressBar(0, N.mat, style = 3)
for(i in 1:N.mat){
  # Update Progress Bar
...
for(i in 1:Depth){
    model %>% layer_dense(units=Height,activation = "relu",input_shape = c(3),kernel_regularizer = regularizer_l1_l2(l1 = l1.pen, l2 = l2.pen)) 
}
# Readout Layer
model %>% layer_dense(units=3)
...
Reservoir<- cbind(NS.samples[,-1],Reservoir) # Add intercept and NS basis functions for numerical stability
