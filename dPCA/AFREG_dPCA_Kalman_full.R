...
{ 
  ...
  r_initAF.Reg.NS <- as.vector(Estimated.OU.Parameters.Betas.PCA$Mu)
  
  # variance of state variable (P0: m x m)
  P_initAF.Reg.NS <- t(Estimated.OU.Parameters.Betas.PCA$Sigma)%*%Estimated.OU.Parameters.Betas.PCA$Sigma
  P_initAF.Reg.NS <- .5*P_initAF.Reg.NS
  
  # intercept of state transition equation (dt: m x 1)
  CAF.Reg.NS <- (Estimated.OU.Parameters.Betas.PCA$Theta)%*%(Estimated.OU.Parameters.Betas.PCA$Mu)
  
  # factor of transition equation (Tt: m x m x 1)
  TtAF.Reg.NS <- -Estimated.OU.Parameters.Betas.PCA$Theta
  
  # factor of measurement equation (Zt: d x m x 1)
  BAF.Reg.NS <- dPCA.samples[,-1]
  ...
  QAF.Reg.NS <- t(Estimated.OU.Parameters.Betas.PCA$Sigma)%*%Estimated.OU.Parameters.Betas.PCA$Sigma
